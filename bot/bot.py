    ...
    try:
        ...
        for result in results:
            try:
                ...
                # Déduire l'ancien groupe depuis le niveau de l'examen
                exam_level = result.exam_id  # L'exam_id correspond au niveau
                
                # Trouver l'ancien groupe en cherchant les rôles actuels
                old_groupe = None
                ...
                if not old_groupe:
                    print(f"⚠️ {member.name} n'a pas de rôle Groupe")
                    old_groupe = "1-A"  # Par défaut
                
                new_groupe = user_db.groupe  # Le nouveau groupe (déjà mis à jour par le site)
                
                print(f"🔍 {member.name}")
                print(f"   Ancien groupe: {old_groupe}")
                print(f"   Nouveau groupe: {new_groupe}")
                
                # SI RÉUSSI ET CHANGEMENT DE GROUPE → Changer les rôles Discord
                if result.passed and old_groupe != new_groupe:
                    ...
                    print(f"   ℹ️ Réussi mais même groupe (pas de changement de rôle)")
