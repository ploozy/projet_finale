...
class CohortManagerSQL:
    ...
    def get_active_formation_cohort(self) -> Cohorte:
        ...
        db = SessionLocal()
        try:
            # Chercher les cohortes en formation
    ...
    def add_user_to_cohort(self, user_id: int, username: str) -> tuple:
        ...
        try:
            ...
            # Trouver ou créer une cohorte en formation
    ...
    def update_user_after_exam(self, user_id: int, passed: bool) -> str:
        ...
        try:
            ...
            if passed:
                ...
                # Échec : transférer vers une cohorte plus récente
                old_cohort_id = user.cohorte_id
                new_cohort = self._find_suitable_cohort_for_retry(user.niveau_actuel, db)
                
                if new_cohort and new_cohort.id != old_cohort_id:
                    ...
                    db.commit()
                    message = f"📚 Transféré vers {new_cohort.id} pour repasser le niveau {user.niveau_actuel}"
                else:
                    db.commit()
                    message = f"❌ Échec - Repassez l'examen lors de la prochaine session"
    ...
    def _find_suitable_cohort_for_retry(self, niveau: int, db: Session) -> Cohorte:
        """Trouve une cohorte appropriée pour un redoublement"""
        ...
        # Sinon chercher une cohorte en formation
        cohort = db.query(Cohorte).filter(
            Cohorte.statut == 'en_formation'
        ).first()
        ...
        # Créer une nouvelle cohorte (fermer la session actuelle d'abord)
        cohort_id = self.create_cohort()
        return db.query(Cohorte).filter(Cohorte.id == cohort_id).first()
